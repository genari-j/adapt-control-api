...
import { Categories, CategoriesRepository } from '../../models/interfaces/categories'

import { env, categoryQuerySchema, createCategoryBodySchema } from '../../validators'
...
export class CategoriesController {
  ...
  async create (request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (categoryByName) { return reply.status(400).send({ error: true, message: 'A categoria informada já existe.' }) }
