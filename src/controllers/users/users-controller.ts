...
import type { ProfilePermissionsRepository } from '../../models/interfaces/profile-permissions'
import type { ProfilesRepository } from '../../models/interfaces/profiles'
import type { CreateUser, UsersRepository } from '../../models/interfaces/users'
...
export class UsersController {
	...
	async updateUserPassword(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (!user) {
				return reply.status(404).send({ error: true, message: 'O Usuário especificado não foi encontrado.' })
			}
			if (password !== confirmPassword) {
				return reply.status(400).send({ error: true, message: 'As senhas especificadas não conferem.' })
			}
			if (password.length < 6) {
				return reply.status(400).send({ error: true, message: 'A senha deve conter no mínimo 6 caracteres.' })
			}
	...
	async solicitationResetPassword(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (!userByEmail) {
				reply.status(404).send({ error: true, message: 'O E-mail especificado não foi encontrado.' })
			}
	...
	async create(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (userByCode) {
				return reply.status(400).send({ error: true, message: 'O código informado já existe.' })
			}

			const userByEmail = await this.usersRepository.findOneBy('email', email)
			if (userByEmail) {
				return reply.status(400).send({ error: true, message: 'Erro ao utilizar este email.' })
			}

			const departmentById = await this.departmentsRepository.findOneBy('id', department_id)
			if (!departmentById) {
				return reply.status(404).send({ error: true, message: 'O departamento informado não existe.' })
			}

			const profileById = await this.profilesRepository.findOneBy('id', profile_id)
			if (!profileById) {
				return reply.status(404).send({ error: true, message: 'O perfil informado não existe.' })
			}
	...
	async getById(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (!userById) {
				return reply.status(404).send({ error: true, message: 'O Colaborador especificado não existe.' })
			}
	...
	async update(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (email) {
				...
				if (userByEmail) {
					return reply.status(400).send({ error: true, message: 'O email informado já existe.' })
				}
				payload.email = email
			}
	...
	async delete(request: FastifyRequest, reply: FastifyReply) {
		try {
			...
			if (!id) {
				return reply.status(400).send({ error: true, message: 'ID do usuário não informado.' })
			}
